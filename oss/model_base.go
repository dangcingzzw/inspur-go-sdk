...
type bucketStoragePolicyOSS struct {
...
type grantOSS struct {
	XMLName    xml.Name       `xml:"Grant"`
	Grantee    granteeOSS     `xml:"Grantee"`
	Permission PermissionType `xml:"Permission"`
	Delivered  bool           `xml:"Delivered"`
}
...
type accessControlPolicyOSS struct {
	XMLName xml.Name   `xml:"AccessControlPolicy"`
	Owner   Owner      `xml:"Owner"`
	Grants  []grantOSS `xml:"AccessControlList>Grant"`
}
