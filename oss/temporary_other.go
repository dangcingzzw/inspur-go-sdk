...
func (OSSClient OSSClient) isSecurityToken(params map[string]string, sh securityHolder) {
	if sh.securityToken != "" {
...
func (OSSClient OSSClient) CreateBrowserBasedSignature(input *CreateBrowserBasedSignatureInput) (output *CreateBrowserBasedSignatureOutput, err error) {
	if input == nil {
	...
	credential, _ := getCredential(sh.ak, OSSClient.conf.region, shortDate)
	...
	if OSSClient.conf.signature == SignatureV4 {
	...
	OSSClient.isSecurityToken(params, sh)
	...
	if OSSClient.conf.signature == SignatureV4 {
		signature = getSignature(policy, sh.sk, OSSClient.conf.region, shortDate)
	} else {
