...
func cleanHeaderPrefix(header http.Header) map[string][]string {
	...
	for key, value := range header {
		if len(value) > 0 {
			...
			if strings.HasPrefix(key, HEADER_PREFIX) || strings.HasPrefix(key, HEADER_PREFIX_OSS) {
...
func convertGrantToXML(grant Grant, isOSS bool, isBucket bool) string {
	xml := make([]string, 0, 4)
	...
	if grant.Grantee.Type == GranteeUser {
		...
		if !isOSS && grant.Grantee.DisplayName != "" {
...
func ConvertLoggingStatusToXml(input BucketLoggingStatus, returnMd5 bool, isOSS bool) (data string, md5 string) {
	grantsLength := len(input.TargetGrants)
	xml := make([]string, 0, 8+grantsLength)

	xml = append(xml, "<BucketLoggingStatus>")
	if isOSS && input.Agency != "" {
	...
	if hasLoggingTarget(input) {
		...
		if grantsLength > 0 {
			...
			for _, grant := range input.TargetGrants {
				xml = append(xml, convertGrantToXML(grant, isOSS, false))
			}
			xml = append(xml, "</TargetGrants>")
		}
...
func ConvertAclToXml(input AccessControlPolicy, returnMd5 bool, isOSS bool) (data string, md5 string) {
	...
	if !isOSS && input.Owner.DisplayName != "" {
...
func convertBucketACLToXML(input AccessControlPolicy, returnMd5 bool, isOSS bool) (data string, md5 string) {
	xml := make([]string, 0, 4+len(input.Grants))
	ownerID := XmlTranscoding(input.Owner.ID)
	xml = append(xml, fmt.Sprintf("<AccessControlPolicy><Owner><ID>%s</ID>", ownerID))
	if !isOSS && input.Owner.DisplayName != "" {
	...
	for _, grant := range input.Grants {
		xml = append(xml, convertGrantToXML(grant, isOSS, true))
	}
	xml = append(xml, "</AccessControlList></AccessControlPolicy>")
	data = strings.Join(xml, "")
	if returnMd5 {
...
func convertTransitionsToXML(transitions []Transition, isOSS bool) string {
...
func convertNoncurrentVersionTransitionsToXML(noncurrentVersionTransitions []NoncurrentVersionTransition, isOSS bool) string {
...
func ConvertLifecyleConfigurationToXml(input BucketLifecyleConfiguration, returnMd5 bool, isOSS bool) (data string, md5 string) {
	...
	for _, lifecyleRule := range input.LifecycleRules {
		...
		if ret := convertTransitionsToXML(lifecyleRule.Transitions, isOSS); ret != "" {
			xml = append(xml, ret)
		}
		if ret := convertExpirationToXML(lifecyleRule.Expiration); ret != "" {
			xml = append(xml, ret)
		}
		if ret := convertNoncurrentVersionTransitionsToXML(lifecyleRule.NoncurrentVersionTransitions, isOSS); ret != "" {
			xml = append(xml, ret)
		}
...
func ConvertEncryptionConfigurationToXml(input BucketEncryptionConfiguration, returnMd5 bool, isOSS bool) (data string, md5 string) {
	xml := make([]string, 0, 5)
...
func converntFilterRulesToXML(filterRules []FilterRule, isOSS bool) string {
...
func converntEventsToXML(events []EventType, isOSS bool) string {
...
func converntConfigureToXML(topicConfiguration TopicConfiguration, xmlElem string, isOSS bool) string {
	xml := make([]string, 0, 6)
	...
	if ret := converntEventsToXML(topicConfiguration.Events, isOSS); ret != "" {
		xml = append(xml, ret)
	}
	if ret := converntFilterRulesToXML(topicConfiguration.FilterRules, isOSS); ret != "" {
		xml = append(xml, ret)
	}
...
// ConverntOSSRestoreToXml converts RestoreObjectInput value to XML data and returns it
func ConverntOSSRestoreToXml(restoreObjectInput RestoreObjectInput) string {
	xml := make([]string, 0, 2)
...
func ConvertNotificationToXml(input BucketNotification, returnMd5 bool, isOSS bool) (data string, md5 string) {
	...
	for _, topicConfiguration := range input.TopicConfigurations {
		ret := converntConfigureToXML(topicConfiguration, "<TopicConfiguration>", isOSS)
		xml = append(xml, ret)
	}
	xml = append(xml, "</NotificationConfiguration>")
	data = strings.Join(xml, "")
...
func parseSseHeader(responseHeaders map[string][]string) (sseHeader ISseHeader) {
	if ret, ok := responseHeaders[HEADER_SSEC_ENCRYPTION]; ok {
		...
		} else if ret, ok = responseHeaders[HEADER_SSEKMS_ENCRYPT_KEY_OSS]; ok {
			sseKmsHeader.Key = ret[0]
		}
...
func ParseGetBucketMetadataOutput(output *GetBucketMetadataOutput) {
	...
	if ret, ok := output.ResponseHeaders[HEADER_VERSION_OSS]; ok {
		output.Version = ret[0]
	}
	if ret, ok := output.ResponseHeaders[HEADER_BUCKET_REGION]; ok {
		output.Location = ret[0]
	} else if ret, ok := output.ResponseHeaders[HEADER_BUCKET_LOCATION_OSS]; ok {
		output.Location = ret[0]
	}
...
func ParseResponseToOSSError(resp *http.Response, isOSS bool) error {
	...
	OSSError := OSSError{}
	respError := ParseResponseToBaseModel(resp, &OSSError, !isJson, isOSS)
	if respError != nil {
		doLog(LEVEL_WARN, "Parse response to BaseModel with error: %v", respError)
	}
	OSSError.Status = resp.Status
	return OSSError
}
...
func convertFetchJobToJSON(input SetBucketFetchJobInput) (data string, err error) {
	...
	for key, value := range input.ObjectHeaders {
		if value != "" {
			...
			if !strings.HasPrefix(key, HEADER_PREFIX_OSS) {
...
func ParseGetBucketFSStatusOutput(output *GetBucketFSStatusOutput) {
	...
	if ret, ok := output.ResponseHeaders[HEADER_FS_FILE_INTERFACE_OSS]; ok {
		output.FSStatus = ParseStringToFSStatusType(ret[0])
	}
}
